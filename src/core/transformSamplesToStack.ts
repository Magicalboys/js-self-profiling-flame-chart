 ...
 */
export function transformSamplesToStack(sourceData: IJsSelfProfilingJSON) {
    const {
        samples, // samples[i]函数执行时间下标，升序
        // resources, // resources[i] 资源uri
        // frames, // frames[i] 函数对象，包含函数名和函数的line和col
        // stacks, // stacks[i] 调用栈记录
    } = sourceData;

    // 遍历samples，为存在 stackId 的 sample 添加 stack 数据结构
    samples.forEach((sample: ISample, i: number) => {
        const {stackId, timestamp} = sample;
        sample.timestamp = getNumberWithToFixed(timestamp); // 保留三位小数
